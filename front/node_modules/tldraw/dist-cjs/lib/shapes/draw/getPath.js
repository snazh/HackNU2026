...
function getPointsFromDrawSegment(segment, scaleX, scaleY, points = []) {
  ...
  if (segment.type === "free" || _points.length < 2) {
    ...
    const pointsToInterpolate = Math.max(4, Math.floor(import_editor.Vec.Dist(_points[0], _points[1]) / 16));
    points.push(...import_editor.Vec.PointsBetween(_points[0], _points[1], pointsToInterpolate));
  }
  return points;
}
